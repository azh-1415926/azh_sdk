    ...
    */
    ...
    public:
        std::string toString() const
        {
            ...
            {
                str += to_string(m_keys_private[i + 1]);
                str += "-";
                str += to_string(m_data_private[i + 1]);
    ...
    private:
        btnode(btnode *parent = nullptr)
            : m_parent_private(parent), m_key_size_private(0)
              ...
              ,
              m_keys_private(new _key_type[_degree + 1]), m_data_private(new _base_type[_degree + 1])
        ...
        static btnode *copyNode(btnode *dest, btnode *src)
        {
            ...
            {
                dest->m_keys_private[i] = src->m_keys_private[i];
                dest->m_data_private[i] = src->m_data_private[i];
            }
        ...
        size_t searchIndex(const _key_type &key) const
        {
            /* 因为下标 0 弃用，从 1 开始检索，当 i 未超出关键字数量且小于 key 的值时，i++ */
            size_t i = 1;
            while (i < m_key_size_private && key > m_keys_private[i])
                i++;
            return i;
        }

        /* 插入数据到指定下标位置，且将关键字数量加一 */
        void insertData(size_t index, const _key_type &key, const _base_type &data)
        {
            ...
            {
                m_keys_private[end_pos - i] = m_keys_private[end_pos - i - 1];
                m_data_private[end_pos - i] = m_data_private[end_pos - i - 1];
            }
            /* 将新关键字、数据插入 index 位置，关键字数量++ */
            m_keys_private[index] = key;
            m_data_private[index] = data;
            m_key_size_private++;
        }
        ...
        _base_type deleteData(size_t index)
        {
            /* 将 index 位置的数据赋值给 data，然后将 index 位置后的所有数据往前移，将会覆盖 index 位置的数据 */
            size_t end_pos = m_key_size_private;
            _base_type data = m_data_private[index];
            for (size_t i = index; i < end_pos; i++)
            {
                m_keys_private[i] = m_keys_private[i + 1];
                m_data_private[i] = m_data_private[i + 1];
            }
    ...
    public:
        ...
        bool insert(const _key_type &key, const _base_type &data)
        {
            ...
            while (curr_node_ptr)
            {
                ...
                if (key > curr_node_ptr->m_keys_private[i])
                {
                    preNode = curr_node_ptr;
                    curr_node_ptr = curr_node_ptr->m_children_private[i];
                }
                else if (key == curr_node_ptr->m_keys_private[i])
                {
                    return false;
                }
                else if (key < curr_node_ptr->m_keys_private[i])
            ...
            i = curr_node_ptr->searchIndex(key);
            if (i == curr_node_ptr->m_key_size_private && key > curr_node_ptr->m_keys_private[i])
                i++;
        ...
        bool erase(_key_type key)
        {
            ...
            if (curr_node_ptr->m_children_private[0] == nullptr)
            {
                ...
                else
                {
                    ...
                    // normal node
                    size_t j = parent->searchIndex(curr_node_ptr->m_keys_private[i]);
                    if (curr_node_ptr->m_keys_private[i] < parent->m_keys_private[j])
                        j--;
                    curr_node_ptr->deleteData(i);
            ...
            else
            {
                ...
                size_t j = parent->searchIndex(target->m_keys_private[target->m_key_size_private]);
                if (target->m_keys_private[target->m_key_size_private] < parent->m_keys_private[j])
                    j--;
                ...
                curr_node_ptr->m_keys_private[i] = target->m_keys_private[target->m_key_size_private];
                curr_node_ptr->m_data_private[i] = target->m_data_private[target->m_key_size_private];
    ...
    private:
        ...
        void splitBTNode(node *n)
        {
            ...
            {
                key = n->m_keys_private[m_s_mid_key_private + i];
                data = n->m_data_private[m_s_mid_key_private + i];
                // n->m_keys_private[m_s_mid_key_private+i]=0;
                // n->data[m_s_mid_key_private+i]=nullptr;
                r_child->insertData(i, key, data);
                r_child->insertChild(i, n->m_children_private[m_s_mid_key_private + i]);
                n->m_children_private[m_s_mid_key_private + i] = nullptr;
            }
            key = n->m_keys_private[m_s_mid_key_private];
            data = n->m_data_private[m_s_mid_key_private];
            if (parent == nullptr)
            {
                ...
                {
                    key = n->m_keys_private[i];
                    data = n->m_data_private[i];
                    // n->m_keys_private[i]=0;
                    // n->data[i]=nullptr;
                    l_child->insertData(i, key, data);
                    l_child->insertChild(i, n->m_children_private[i]);
                    n->m_children_private[i] = nullptr;
                }
                key = n->m_keys_private[m_s_mid_key_private];
                data = n->m_data_private[m_s_mid_key_private];
                // n->m_keys_private[m_s_mid_key_private]=0;
                // n->data[m_s_mid_key_private]=nullptr;
                n->m_key_size_private = 0;
                n->insertChild(0, l_child);
                n->insertData(1, key, data);
            ...
            else
            {
                ...
                // n->m_keys_private[m_s_mid_key_private]=0;
                // n->data[m_s_mid_key_private]=nullptr;
                if (key > parent->m_keys_private[index])
                    index++;
        ...
        void moveLeft(node *parent, size_t index, node *l_child, size_t rm_pos)
        {
            ...
            if (l_child != nullptr)
            {
                ...
                {
                    l_child->m_keys_private[i] = l_child->m_keys_private[i + 1];
                    l_child->m_data_private[i] = l_child->m_data_private[i + 1];
                }
                if (rm_pos > l_child->m_key_size_private)
                {
                    l_child->m_key_size_private++;
                }
                l_child->m_keys_private[l_child->m_key_size_private] = parent->m_keys_private[index];
                l_child->m_data_private[l_child->m_key_size_private] = parent->m_data_private[index];
            }
            l_child->insertChild(l_child->m_key_size_private, r_child->deleteBTNode(0));
            // parent move to left
            parent->m_keys_private[index] = r_child->m_keys_private[1];
            parent->m_data_private[index] = r_child->m_data_private[1];
            // right child move to left
            for (size_t i = 1; i < r_child->m_key_size_private; i++)
            {
                r_child->m_keys_private[i] = r_child->m_keys_private[i + 1];
                r_child->m_data_private[i] = r_child->m_data_private[i + 1];
            }
            // r_child->m_keys_private[r_child->m_key_size_private]=0;
            // r_child->data[r_child->m_key_size_private]=nullptr;
        ...
        void moveRight(node *parent, size_t index, node *r_child, size_t rm_pos)
        {
            ...
            if (r_child != nullptr)
            {
                ...
                r_child->m_keys_private[1] = parent->m_keys_private[index];
                r_child->m_data_private[1] = parent->m_data_private[index];
            }
            r_child->insertChild(0, l_child->deleteBTNode(l_child->m_key_size_private));
            // parent move to left
            parent->m_keys_private[index] = l_child->m_keys_private[l_child->m_key_size_private];
            parent->m_data_private[index] = l_child->m_data_private[l_child->m_key_size_private];
            // left child move to left
            //  l_child->m_keys_private[l_child->m_key_size_private]=0;
            //  l_child->data[l_child->m_key_size_private]=nullptr;
            l_child->m_key_size_private--;
        }
        ...
        void combineBTNode(node *parent, size_t index)
        {
            ...
            if (l_child->m_key_size_private < m_s_min_key_private)
            {
                ...
                l_child->insertData(l_child->m_key_size_private + 1, parent->m_keys_private[index], parent->m_data_private[index]);
                ...
                for (size_t i = 1; i <= r_child->m_key_size_private; i++)
                {
                    key = r_child->m_keys_private[i];
                    data = r_child->m_data_private[i];
                    l_child->insertData(l_child->m_key_size_private + 1, key, data);
            ...
            else if (r_child->m_key_size_private < m_s_min_key_private)
            {
                ...
                r_child->insertData(1, parent->m_keys_private[index], parent->m_data_private[index]);
                r_child->insertChild(0, l_child->m_children_private[0]);
                for (size_t i = 1; i <= l_child->m_key_size_private; i++)
                {
                    key = l_child->m_keys_private[i];
                    data = l_child->m_data_private[i];
                    r_child->insertData(1, key, data);
            ...
            if (parent->m_parent_private == nullptr)
            {
                ...
                if (parent->m_key_size_private == 0)
                {
                    ...
                    {
                        parent->m_keys_private[i] = n->m_keys_private[i];
                        parent->m_data_private[i] = n->m_data_private[i];
            ...
            else
            {
                ...
                size_t j = parent->searchIndex(n->m_keys_private[index]);
                if (n->m_keys_private[index] < parent->m_keys_private[j])
                    j--;
                n->deleteData(index);
